...
Fixes in this version:
  • Role Overrides  (Stone=Tank regardless of Raid-Helper sign-up)
  • Smart multi-day assignment  (flexible players go where their role is MISSING)
...
DEFAULT_FIXED    = "Stone=Tuesday\nPumpyy=Tuesday"
DEFAULT_OVERRIDES = "Stone=Tank"   # Role override — name=Role
...
def parse_signups(event_data: dict, day_idx: int, strict: bool,
                  role_overrides: dict) -> list:
    """
    role_overrides: {name_lower: "Tank"|"Healer"|"DPS"}
    Overrides the role from Raid-Helper for specific players.
    """
    ...
    players  = []
    for s in signups:
        if str(s.get("status","")).lower().strip() not in statuses:
            continue
        ...
        # Apply role override if defined
...
def parse_role_overrides(raw: str) -> dict:
    ...
    for line in raw.strip().splitlines():
        ...
        name = name.strip().lower(); role = role.strip().lower()
...
def build_all_raids(
    players_by_day:    dict,   # {day_idx: [Player, …]}
    fixed_assignments: dict,   # {name_lower: day_idx}
    buddy_groups:      list,   # [{name_lower, …}, …]
) -> dict:
    """
    Key improvements over previous version:
    1. Raid slot count based on EXCLUSIVE sign-ups (only-this-day), not raw total.
       This avoids creating a 2nd Monday group just because flex players are also signed there.
    2. Flexible players (multi-day) are deferred and assigned in a 2nd pass to the day
       where their role is still MISSING from the roster.
    3. Labels are always derived from (day_idx, slot) — never from a global counter.
    """

    # ── 1. Merge duplicates across days
    seen: dict = {}
    ...
    # ── 2. Apply fixed day assignments (hard lock)
    ...
    # ── 3. Apply buddy groups (soft — restrict to common days if they exist)
    ...
    # ── 4. Compute exclusive counts per day (= players with ONLY that day available)
    #       Use this for the 2nd-group threshold — much more accurate than raw sign-up count.
    exclusive_count: dict = defaultdict(int)
    for p in all_players:
        if len(p.avail_days) == 1:
            exclusive_count[p.avail_days[0]] += 1

    # A day gets a 2nd slot only if it has ≥18 EXCLUSIVE players.
    # Always try to reach 3 total slots across the week.
    ...
    # Boost to reach 3 total
    total  = sum(raids_per_day.values())
    active = sorted([d for d in range(3) if raw_count.get(d,0) >= 10],
    ...
    # ── 5. Pre-build slot labels (derived purely from day_idx + slot)
    slot_labels: list[tuple] = []   # [(day_idx, label), …]
    ...
    # ── 6a. FIRST PASS — assign exclusive players (only one day available)
    #        This fills the skeleton of each raid with players who MUST be here.
    for day_idx, label in slot_labels:
        pool = [p for p in all_players
                if len(p.avail_days) == 1
                and p.avail_days[0] == day_idx
                and not p.assigned]

        # Fixed-day players first within the exclusive pool
        fixed_here = [p for p in pool
                      if p.name_lower in fixed_assignments
                      and fixed_assignments[p.name_lower] == day_idx]
        others     = [p for p in pool if p not in fixed_here]

        def rq_excl(role):
            return [p for p in fixed_here if p.role == role] + \
                   [p for p in others     if p.role == role]
    ...
    # ── 6b. SECOND PASS — assign flexible players (multi-day available)
    #        For each flexible player, find the slot where their role is MOST NEEDED.
    flexible = [p for p in all_players if not p.assigned and len(p.avail_days) > 1]

    def _need(role: str, label: str) -> int:
        """How many more of this role does a group still need?"""
        have = sum(1 for x in results[label] if x.role == role)
        return max(0, TARGET[role] - have)

    def _group_full(label: str) -> bool:
        return len(results[label]) >= RAID_SIZE

    # Sort flexible players: fixed-day first, then by role scarcity
    def _flex_priority(p: Player) -> tuple:
        is_fixed = p.name_lower in fixed_assignments
        return (0 if is_fixed else 1, 0 if p.role == "Tank" else (1 if p.role == "Healer" else 2))

    flexible.sort(key=_flex_priority)

    for p in flexible:
        if p.assigned:
            continue

        # Candidate labels: slots on days this player is available, not yet full
        candidates = [(day_idx, lbl) for day_idx, lbl in slot_labels
                      if day_idx in p.avail_days and not _group_full(lbl)]

        if not candidates:
            continue

        # Prioritise the slot that NEEDS this player's role most
        # Tiebreak: slot with fewer total players (more room)
        def _score(entry):
            _, lbl = entry
            need  = _need(p.role, lbl)
            size  = len(results[lbl])
            return (-need, size)   # higher need = lower score = picked first
